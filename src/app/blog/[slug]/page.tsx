...
function renderContent(content: string) {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith("## ")) {
      elements.push(
        <h2 key={i} style={{
          color: "var(--wr-text)", fontSize: 24, fontWeight: 700,
          margin: "40px 0 16px", borderLeft: "3px solid var(--wr-green)", paddingLeft: 16,
        }}>
          {line.replace("## ", "")}
        </h2>
      );
      ...
      elements.push(
        <p key={i} style={{ color: "var(--wr-green)", fontWeight: 700, fontSize: 16, margin: "20px 0 6px" }}>
          {line.replace(/\*\*/g, "")}
        </p>
      );
      ...
      if (match) {
        ...
        elements.push(
          <div key={i} style={{ margin: "36px 0" }}>
            ...
            />
            {caption && (
              <p style={{ color: "var(--wr-text-faint)", fontSize: 13, textAlign: "center", marginTop: 10, fontStyle: "italic" }}>
                {caption}
              </p>
            )}
          </div>
      ...
      elements.push(
        <div key={`table-${i}`} style={{ overflowX: "auto", margin: "24px 0" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
            <thead>
              <tr style={{ background: "var(--wr-card-hover)" }}>
                {headers.map((h, j) => (
                  <th key={j} style={{ padding: "12px 16px", textAlign: "left", color: "var(--wr-green)", fontWeight: 600, borderBottom: "1px solid var(--wr-border-strong)" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {rows.map((row, ri) => (
                <tr key={ri} style={{ background: ri % 2 === 0 ? "var(--wr-card)" : "var(--wr-bg-alt)" }}>
                  {row.map((cell, ci) => (
                    <td key={ci} style={{ padding: "12px 16px", color: cell.startsWith("**") ? "var(--wr-text)" : "var(--wr-text-2)", borderBottom: "1px solid var(--wr-border)", fontWeight: cell.startsWith("**") ? 700 : 400 }}>
                      {cell.replace(/\*\*/g, "")}
                    </td>
                  ))}
                </tr>
      ...
      elements.push(
        <p key={i} style={{ color: "var(--wr-text-2)", fontSize: 16, lineHeight: 1.85, margin: "0 0 18px" }}>
          {line}
        </p>
      );
    }
    ...
    <>
      ...
      <main style={{ background: "var(--wr-bg)", minHeight: "100vh", paddingTop: 64, transition: "background 0.2s" }}>
        ...
        <div style={{ position: "relative", height: 420, overflow: "hidden" }}>
          ...
          <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "0 24px 48px" }}>
            <div style={{ maxWidth: 820, margin: "0 auto" }}>
              <Link href="/blog" style={{ color: "rgba(255,255,255,0.6)", fontSize: 13, textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 6, marginBottom: 20 }}>
                ← Back to Blog
              </Link>
              <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap", alignItems: "center" }}>
                ...
                <span style={{ color: "rgba(255,255,255,0.5)", fontSize: 13 }}>{post.readTime}</span>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "48px 24px 80px", display: "grid", gridTemplateColumns: "1fr 300px", gap: 48 }}>
          ...
          {/* Article */}
          <article>
            <p style={{ color: "var(--wr-text-muted)", fontSize: 17, lineHeight: 1.8, margin: "0 0 32px", borderLeft: "3px solid var(--wr-green)", paddingLeft: 20, fontStyle: "italic" }}>
              {post.excerpt}
            </p>
            ...
            <div style={{
              ...
              border: "1px solid var(--wr-green)", borderRadius: 16,
              padding: "32px", marginTop: 48, textAlign: "center",
            }}>
              <h3 style={{ color: "var(--wr-text)", fontSize: 22, fontWeight: 700, margin: "0 0 10px" }}>
                Ready to book your next adventure?
              </h3>
              <p style={{ color: "var(--wr-text-muted)", fontSize: 15, margin: "0 0 24px" }}>
          ...
          <aside style={{ position: "sticky", top: 84, height: "fit-content" }}>
            {/* Share */}
            <div style={{ background: "var(--wr-card)", border: "1px solid var(--wr-border)", borderRadius: 14, padding: 24, marginBottom: 24, transition: "background 0.2s" }}>
              <p style={{ color: "var(--wr-text)", fontWeight: 700, fontSize: 15, margin: "0 0 16px" }}>Share this article</p>
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {[
                  { label: "Share on WhatsApp", color: "#25D366", bg: "#1a2e1a" },
                  { label: "Share on Twitter/X", color: "#1DA1F2", bg: "#1a1f2e" },
                  { label: "Copy link", color: "var(--wr-text-muted)", bg: "var(--wr-card-hover)" },
                ].map(({ label, color, bg }) => (
            ...
            {/* Related Posts */}
            <div style={{ background: "var(--wr-card)", border: "1px solid var(--wr-border)", borderRadius: 14, padding: 24, transition: "background 0.2s" }}>
              <p style={{ color: "var(--wr-text)", fontWeight: 700, fontSize: 15, margin: "0 0 16px" }}>More articles</p>
